...
class SearchBook:
    ...
    # bige7搜索
    def bige7(self):
        response = requests.get("https://www.bqg70.com/s?q=" + quote(self.book), headers={"User-Agent": getRandomUserAgent()})
...
class CrawlBook:
    ...
    def getBiGe7CrawlUrls(self):
        ...
        response = requests.get(self.bookUrl, headers={"User-Agent": getRandomUserAgent()})
    ...
    def crawlBiGe7SingleBook(self, thread=10, verbose=True, path='./'):
        ...
        with Pool(thread) as p:
            allChapterContent = list(tqdm(p.imap(self.getBiGe7SpotSingleChapterTitleContent, bookChapterUrlList[:-10]), total=len(bookChapterUrlList[:-10])))
...
def tableShowBookInfo(bookInfo):
    ...
    for info in bookInfo:
        table.add_row(['《' + info['bookName'] + '》', info['author'].split('：')[1], info['bookUrl'].split('/')[-2], info['source']])
...
def argparse_deal():
    ...
    if args.search:
        ...
        CrawlBook(urljoin("https://www.bqg70.com/book/", args.book)).crawlBiGe7SingleBook(args.thread, args.verbose, args.path)
    else:
        print('[ERROR] 请输入参数')
